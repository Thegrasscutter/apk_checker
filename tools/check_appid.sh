...
if [ -n "$1" ]; then
    ...
    LOG_FILE="../output/singlescans/${1//[:\/]/_}-appid_check_logs.txt"

    if [[ $1 =~ ^1:[0-9]+:(android|ios):[a-zA-Z0-9]+$ ]]; then
        echo "[*] Valid Firebase appID format supplied: $1" | tee -a "$LOG_FILE"
    else
        echo "[-] Invalid Firebase appID format: $1" | tee -a "$LOG_FILE"
        echo "[*] Expected usage: 1:PROJECT_NUMBER:(android|ios):APP_IDENTIFIER GCP_API_KEY" | tee -a "$LOG_FILE"
        exit 1
    fi

    if [[ $2 =~ ^AIza[a-zA-Z0-9\-_]{35}$ ]]; then
        echo "[*] Valid GCP API key format supplied: $2" | tee -a "$LOG_FILE"
        API_KEYS=("$2")
    else
        echo "[-] Invalid GCP API key format: $2" | tee -a "$LOG_FILE"
        echo "[*] Expected usage: 1:PROJECT_NUMBER:(android|ios):APP_IDENTIFIER GCP_API_KEY" | tee -a "$LOG_FILE"
        exit 1
    fi

    echo "[*] Starting Firebase AppID check for $1 with GCP API key $2" | tee -a "$LOG_FILE"

    # Create both Android and iOS variants for the supplied app ID
    echo "[*] Generating Android and iOS variants for supplied app ID..." | tee -a "$LOG_FILE"
    APP_IDS=""
    project_part=$(echo "$1" | sed 's/1:\([0-9]*\):\(android\|ios\):[a-zA-Z0-9]*/1:\1/')
    app_identifier=$(echo "$1" | sed 's/1:\([0-9]*\):\(android\|ios\):\([a-zA-Z0-9]*\)/\3/')
    ...
    # Process the API key to get projectID from Firebase check files
    echo "[*] Processing API key and extracting projectID..." | tee -a "$LOG_FILE"

    api_key="$2"
    echo "[*] Processing API key: $api_key" | tee -a "$LOG_FILE"
    # Check if Firebase check file exists for this API key
    firebase_check_file="../output/singlescans/firebase_check_${api_key}.json"
    if [[ ! -f "$firebase_check_file" ]]; then
        echo "[-] Firebase check file not found for key $api_key: $firebase_check_file" | tee -a "$LOG_FILE"
        echo "[-] Please ensure check_firebase.sh has been run first" | tee -a "$LOG_FILE"
        exit 1
    fi
    ...
    if [[ -z "$project_id" || "$project_id" == "null" ]]; then
        ...
        echo "[-] The Firebase check file may be invalid or incomplete" | tee -a "$LOG_FILE"
        exit 1
    fi
    ...
    echo "$APP_IDS" | while read -r app_id; do
        ...
        for app_instance_id in "PROD" "DEV" "TEST"; do
            ...
            # Log the result
            result_file="../output/singlescans/firebase_remote_config_${api_key}_${project_id}_$(echo "$app_id" | tr ':' '_').json"
