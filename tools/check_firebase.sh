...
if [ -n "$1" ]; then
    echo "[*] Analyzing Firebase API key: $1 with Agneyastra" | tee  ../output/singlescans/firebase_check_logs.txt
    cd /tools/agneyastra_py || { echo "Directory /tools/agneyastra_py not found!"; exit 1; }
    python3 agneyastra.py --key $1 --report_path /app/agneyastra_report_$1.html 2>&1 | tee -a ../output/singlescans/$1-firebase_check_logs.txt
    exit_code=$?
    echo "[DEBUG] Agneyastra exit code: $exit_code" | tee -a ../output/singlescans/$1-firebase_check_logs.txt
    if [ -s ../output/singlescans/agneyastra_report_$1.html ]; then
        if grep -q "vulnerable:true" ../output/singlescans/agneyastra_report_$1.html; then
            echo -e "\e[31m[!] Vulnerable Firebase API key found: $1 \n Check ../output/singlescans/agneyastra_report_$1.html for details.\e[0m" | tee -a ../output/singlescans/$1-firebase_check_logs.txt
        else
            echo "[+] No vulnerabilities found for Firebase API key: $1" | tee -a ../output/singlescans/$1-firebase_check_logs.txt
        fi
    fi
else
