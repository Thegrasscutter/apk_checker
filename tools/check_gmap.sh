...
if [ -n "$1" ]; then
    echo "[*] Scanning API key: $1" | tee ../output/singlescans/$1-gmap_scan_results.txt
    echo "----------------------------------------" | tee -a ../output/singlescans/$1-gmap_scan_results.txt
    yes | gmapsapiscanner --api-key $1 >> ../output/singlescans/$1-gmap_scan_results.txt

    if [ -s ../output/singlescans/$1-gmap_scan_results.txt ]; then
        if grep -q "API key is  vulnerable" ../output/singlescans/$1-gmap_scan_results.txt; then
            echo -e "\e[31m[!] Vulnerable Google Maps API keys found! Check /app/$1-gmap_scan_results.txt for details.\e[0m" | tee -a /app/gmap_scan_logs.txt
        else
            echo "[*] No vulnerable Google Maps API keys found."
        fi
    fi
else
